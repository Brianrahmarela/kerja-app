  ...
  componentDidMount() {
    getSkills()
      .then((res: AxiosResponse<any>) => {
        ...
        this.setState({
          beginner: res.data
            .filter((val: any) => val.level === "BEGINNER")
            .map((val: any) => val.skillName),
          intermediate: res.data
            .filter((val: any) => val.level === "INTERMEDIATE")
            .map((val: any) => val.skillName),
          advance: res.data
            .filter((val: any) => val.level === "ADVANCE")
            .map((val: any) => val.skillName),
        });
